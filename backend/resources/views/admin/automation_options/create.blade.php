...
@section('title', 'Новая опция автоматики')

@section('content')
<div class="bg-white rounded-lg border border-gray-200 p-6 max-w-2xl">
    <form action="{{ route('admin.automation-options.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle') }}" class="w-full border border-gray-300 rounded px-3 py-2 focus:ring-2 focus:ring-stone-500 focus:border-stone-500">
        </div>

        <div>
            <label for="icon" class="block text-sm font-medium text-gray-700">Иконка</label>
            <input type="text" name="icon" id="icon" value="{{ old('icon') }}" class="w-full border border-gray-300 rounded px-3 py-2 focus:ring-2 focus:ring-stone-500 focus:border-stone-500">
        </div>
